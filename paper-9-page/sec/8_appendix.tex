...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figures/figure_3_rejected_examples.png}
  \caption{Three representative rejected counterfactuals from the
  audited-but-rejected set. From top: crosswalk repainting adds a new
  foreground zebra crossing rather than repairing the supported
  intersection marking (Santiago), localised greenery addition places
  shrubs in the wrong roadside verge (Abuja), and storefront
  transparency increase turns the target windows into opaque black
  openings rather than revealing a plausible interior (Amsterdam).}
  \label{fig:rejected_qualitative}
\end{figure}
...
preservation.  Representative rejected counterfactuals are shown in
Figure~\ref{fig:rejected_qualitative}. Generator failure before audit
is zero in the cleaned
